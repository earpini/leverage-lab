# Where AI Lands / Ettore Arpini — brand for coding agents

Paste this whole file into your project (e.g. as `BRAND.md`) and tell your coding
agent: **"Follow BRAND.md. Use these CSS variables for all color, type, spacing,
radius, and motion. Do not invent new hex values, fonts, or radii."**

Drop the stylesheet below into the app once, then everything references `var(--ea-*)`.

---

## 1. The stylesheet — paste into `brand.css` and link it
...
## 2. The rules an agent must follow

**Voice & tone** — Argument-led, calm urgency; a policy strategist, not a hot-take account.
Concrete over abstract. **Sentence case everywhere** — UPPERCASE only for small,
wide-tracked eyebrow labels (`--ea-tracking-label`, `--ea-gray`).

**Color discipline**
- One accent: **clay**. Links/small text use `--ea-clay-ink` for contrast on paper.
- **Gold is a spark** — one highlight, one number, the dash on dark. Never body, never links.
- Slate is the *cool/analytical* temperature — use it for a run of analytical pieces, not mixed in.
- Depth comes from tints (`*-soft`), never from new hues.

**Type**
- Headlines & wordmark → `--ea-font-display` (Helvetica Neue), weight 800–900, tight tracking.
- Body/UI/labels → `--ea-font-ui` (Hanken Grotesk).
- Long-form reading & pull-quotes → `--ea-font-serif` (Source Serif 4).

**Shape & space** — Square-edged and confident: small radii only. Generous air ("let it breathe").
Structure with **rules and space**, not boxes and shadows. Reading column ≈ 680px.

**Imagery** — Grounded, documentary, warm clay→gold duotone. **Never** robot hands,
glowing brains, neon-tech, or circuit motifs.

**Motion** — Quick, decisive, no bounce. Hover deepens the accent; press shifts darker
(no shrink); focus = 2px clay ring at 2px offset.
